        ...
        <form [formGroup]="workoutForm" (ngSubmit)="onSubmit()" class="workout-form">
          ...
          <div class="form-actions">
            ...
            <button type="submit" [disabled]="!workoutForm.valid || loading" class="btn btn-primary">
              <span *ngIf="!loading">
                {{ workoutId ? 'Update Workout' : 'Log Workout' }}
              </span>
              <span *ngIf="loading">
...
export class WorkoutEditComponent implements OnInit {
  ...
  onSubmit() {
    ...
    this.workoutService.updateWorkout(this.workoutId, payload).subscribe({
      next: () => {
        this.loading = false;
        this.notificationService.success('Workout updated successfully 💪');

        this.router.navigate(['/workouts', this.workoutId]);
      },
      error: (err) => {
        this.loading = false;
        const message = err.error?.error?.message || 'Update failed';
        this.error = message;
